...
class RNN(object):
    ...
    def __init__(self, num_emb, emb_dim, hidden_dim,
        ...
        samples = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length)
        ...
        d_gen_predictions = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length, dynamic_size=False, infer_shape=True)
        d_real_predictions = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length, dynamic_size=False, infer_shape=True)

        self.gen_x = self.gen_x.stack()
        emb_gen_x = tf.gather(self.d_embeddings, self.gen_x)
        ta_emb_gen_x = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length)
        ta_emb_gen_x = ta_emb_gen_x.unstack(emb_gen_x)

        emb_real_x = tf.gather(self.d_embeddings, self.x)
        ta_emb_real_x = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length)
        ...
        _, _, _, self.d_gen_predictions = control_flow_ops.while_loop(
            ...
            body=_d_recurrence,
            loop_vars=(tf.constant(0, dtype=tf.int32), ta_emb_gen_x, self.d_h0, d_gen_predictions))
        self.d_gen_predictions = tf.reshape(self.d_gen_predictions.stack(), [self.sequence_length])

        _, _, _, self.d_real_predictions = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3: i < self.sequence_length,
            body=_d_recurrence,
            loop_vars=(tf.constant(0, dtype=tf.int32), ta_emb_real_x, self.d_h0, d_real_predictions))
        self.d_real_predictions = tf.reshape(self.d_real_predictions.stack(), [self.sequence_length])
        ...
        _, _, _, self.g_predictions = control_flow_ops.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32), tf.gather(self.g_embeddings, self.start_token), self.h0, g_predictions))

        self.g_predictions = tf.reshape(self.g_predictions.stack(), [self.sequence_length, self.num_emb])
        ...
        self.d_real_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.d_real_predictions, labels=tf.ones([self.sequence_length])))
        ...
        # pretraining loss
        self.pretrain_loss = (-tf.reduce_sum(tf.one_hot(tf.to_int64(self.x), self.num_emb, 1.0, 0.0) * tf.log(self.g_predictions)) / self.sequence_length)
    ...
    def create_output_unit(self, params, embeddings):
        ...
        def unit(h_t):
            logits = tf.reshape(self.b_out2 + tf.matmul(embeddings, tf.tanh(self.b_out1 + tf.matmul(self.W_out, tf.reshape(h_t, [self.hidden_dim, 1])))), [1, self.num_emb])
...
class GRU(RNN):
    ...
    def create_recurrent_unit(self, params):
        ...
        params.extend([self.W_rx, self.W_zx, self.W_hx, self.U_rh, self.U_zh, self.U_hh])
