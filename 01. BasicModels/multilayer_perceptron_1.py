...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            batch_x, batch_y =  [input_data[i]], [label_data[i]]
    ...
    print ("Accuracy:", accuracy.eval({x: input_data, y: label_data}))

    result1, result2 = sess.run([pred, tf.arg_max(pred, 1)], feed_dict={x: input_data})
    print (result1)
    print (result2)
